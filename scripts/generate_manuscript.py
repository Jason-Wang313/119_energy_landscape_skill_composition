...
def make_manuscript(summary):
    ...
    a(r"\begin{abstract}")
    a(
        "A skill library becomes reliable when a planner predicts what one skill enables next and adapts later choices from the outcome. "
        "A terminal state may fall outside the next skill's basin, cross a high-energy barrier, or enter a contact mode with nonsmooth descent. "
        "We treat that handoff as a local instance of world/action modeling: a compact interface that predicts a skill transition's physical consequence before the planner commits. Here the modeled state is the seam, and the actions are transition, repair, probe, abstention, and alternate-edge choice. Given terminal evidence, next-skill basin/descent estimates, and a candidate transition, the interface predicts whether action/skill composition will fail, diagnoses why, chooses accept, repair, probe, abstain, or another transition, and updates planner-edge memory for future plans. "
        ...
        "The current evidence supports a bounded claim: a local predictive world/action model for skill-seam consequences improves composition when basin, barrier, and descent quantities are estimable and their outcomes update future planning. This is not a full robot world model; external robot or high-fidelity validation remains necessary before deployment-level claims."
    )
    a(r"\end{abstract}")
    ...
    a(
        ...
        "Modern robot-learning systems add broad skill libraries and large robot datasets \\citep{florence2022implicit,brohan2023rt1,openx2023}. The gap targeted here is not low-level skill learning or a new manipulation controller; it is the missing compact predictive interface between a skill library and a planner, and specifically an action-conditioned physical interface between a skill library and a planner. Before execution, the robot should estimate whether a proposed edge will land inside the next skill's feasible start set or whether it should repair, probe, abstain, or choose a different transition. After execution, it should keep the outcome as planning evidence: which handoff was reliable, which needed repair, which called for abstention, which should be avoided later, and which missing bridge skill would make future plans less brittle."
    )
    a(
        "The core failure mode is simple: skill one succeeds, but its terminal distribution lands near a ridge or outside the basin of skill two. A module graph may mark the edge legal, while execution would require a high-energy repair, cross a barrier, or switch contact modes so the next controller no longer descends. We ask whether an explicit seam model can make composition more reliable by predicting failed transitions before execution, diagnosing the likely failure source, choosing repair, probe, abstain, or an alternate transition when acceptance is unsafe, and using the outcome to bias future planning. Contact-rich examples are useful here because they make action consequences hard to ignore; they are a testbed for the seam model, not the identity of the paper."
    )
    a(
        "The broader motivation is adaptive physical world/action models for embodied agents: understanding how a robot represents the physical consequences of action, notices when that representation is incomplete, and adapts future planning from the outcome. We use the world/action-model view at a deliberately local scale: the model is not a whole robot simulator, and the prediction-action-update loop is limited to the handoff. This makes the seam a local world/action-modeling problem rather than a full-system modeling claim. That restraint is intentional. Rather than renaming every controller check as a world model, the paper studies the small interface where a planner asks, before committing to the next skill, what this transition is likely to do, why it might fail, which response is warranted, and what should be remembered afterward."
    )
    a(
    ...
    a(
        ...
        r"The action set at this boundary is deliberately planner-facing: accept the edge, repair the handoff, probe an uncertain seam, abstain, or route to an alternate transition."
    )
    a(
    ...
    a(
        ...
        r"Here, local physical predictions become action choices, and action outcomes become future planning evidence."
    )
    ...
    a(
        ...
        f"Ablations add {counts['ablation_cell']:,} cells, stress sweeps add {counts['stress_cell']:,} cells, fixed-risk tests add {counts['fixed_risk_cell']:,} cells, and the failure audit contains {counts['failure_cases']} cases. The previous proposed method remains a named baseline. The protocol measures both composition outcomes and world/action-model behavior at the seam: predicted risk, diagnostic label, decision, realized breach, and planner-edge update."
    )
    ...
    a(
        ...
        "The natural boundary is scale and role: this paper is not a full behavior-composition stack, action tokenizer, foundation policy, or simulation world model. It is a seam-level reliability layer meant to sit beside such systems and improve future planning by turning handoff predictions and failures into updated planner edge beliefs. The connection is intentionally narrow: physical prediction is local, actions are skill transitions, and adaptation happens through planner memory rather than through a claim of general-purpose simulation."
    )
    ...
    a("External robot or high-fidelity validation remains necessary before deployment-level claims.")
    a(
