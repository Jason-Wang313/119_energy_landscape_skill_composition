...
- The ManiSkill render-video preflight records that render-backed RGB MP4 evidence-video export is separately audited with explicit render-backend/shader controls and a renderer-failure classifier and currently blocked locally, so diagnostic fallback videos cannot be confused with external validation.
