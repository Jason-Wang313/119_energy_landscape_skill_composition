...
\begin{center}
{\LARGE \textbf{Local World/Action Models for Robot Skill Seams}}\\
...
\textbf{What this is.}
\begin{itemize}
\item A local skill-seam interface for world/action modeling.
...
\item \textbf{Alternate transition}: use another edge or ask for a new bridge skill.
\end{itemize}
...
The proposed composer improves hard success by 0.08460 and hard utility by 0.23517 against the strongest non-oracle predecessor. It also reduces seam failure, barrier violation, damage, composition cost, risk calibration error, and realized seam breach while improving basin alignment and descent continuity. A diagnostic audit checks exported failure labels, seam decisions, and planner-edge updates over 230,400 local rows, and a failure-memory adaptation audit checks whether early diagnostic/update signatures predict held-out seam outcomes locally, so the mechanism evidence is not only aggregate success.
