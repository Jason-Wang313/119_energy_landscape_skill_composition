...
- Added `scripts/audit_maniskill_render_video_preflight.py` so the tracked ManiSkill/SAPIEN route separately audits render-backed RGB MP4 export before official collection and records the current local renderer blocker without treating diagnostic fallback videos as evidence.
