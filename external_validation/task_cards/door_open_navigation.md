# Task Card: door_open_navigation

Not external evidence: `true`.

Seam under test: contact-mode transition must not leave the next navigation skill outside its feasible start set.
Skill pair: `open_door_to_passage` -> `navigate_through_doorway`.
Platform type: `high_fidelity_sim`.
Required paired resets per method: `30`.
Expected log: `external_validation/logs/door_open_navigation.jsonl`.
Expected videos: `external_validation/videos/door_open_navigation`.

## Fidelity Checks

- door hinge dynamics
- doorway clearance
- robot base pose logging
