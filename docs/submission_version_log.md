...
- Added `scripts/audit_maniskill_pilot_runtime_liveness.py` as a ManiSkill pilot runtime liveness audit that launches the tracked reference runner in quarantined output directories with a hard timeout; the current local report records `pilot_runtime_ready=false`, one diagnostic fallback row/video, explicit render-backend/shader controls, and a concrete renderer failure summary before official collection.
...
- Updated the external collection plan and runbook audit so the operator runbook now carries 42 validation commands, the current ManiSkill route gates, and the actual high-fidelity collection command template before strict evidence gates.
...
- Added `scripts/self_test_external_adapter_evidence.py` so strict manifest-declared adapter evidence is tested on temporary complete implementations while missing manifests, scaffold templates, and reference adapters remain fail-closed through a strict reference-adapter rejection gate.
