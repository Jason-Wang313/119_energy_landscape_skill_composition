...
- Use the independent operator packet, strict MP4 video evidence gate, official video write guard, official JSONL write guard, atomic official JSONL promotion, External ablation collection packet, External evidence intake ledger, reference-adapter provenance catalog, strict reference-adapter rejection gate, strict independent method provenance gate, strict manifest promotion gate, and manifest assembly checklist as Jason-owned validation scaffolding; do not frame Haonan as responsible for supplying the missing proof.
