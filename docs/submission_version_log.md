...
- Added `scripts/audit_maniskill_pilot_runtime_liveness.py` as a ManiSkill pilot runtime liveness audit that launches the tracked reference runner in quarantined output directories with a hard timeout; the current local report records `pilot_runtime_ready=false`, zero rows, zero videos, and a concrete renderer failure summary before official collection.
