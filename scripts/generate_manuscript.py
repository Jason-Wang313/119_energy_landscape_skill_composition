...
def make_manuscript(summary):
    ...
    a(
        "The core failure mode is simple: skill one succeeds, but its terminal distribution lands near a ridge or outside the basin of skill two. A module graph may mark the edge legal, while execution would require a high-energy repair, cross a barrier, or switch contact modes so the next controller no longer descends. We ask whether an explicit seam model can make composition more reliable by predicting failed transitions before execution, diagnosing the likely failure source, choosing repair, probe, abstain, or an alternate transition when acceptance is unsafe, and using the outcome to bias future planning. Contact-rich examples are useful here because they expose action consequences that a motion-only edge label misses; they are a testbed for the seam model, not the identity of the paper."
    )
    a(
