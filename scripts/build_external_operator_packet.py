...
def write_md(payload: dict[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            f"- Current checkout commit: `{materializer['current_checkout'].get('code_commit', '')}`",
            f"- Current skill-library hash: `{materializer['current_checkout'].get('skill_library_hash', '')}`",
            f"- Current checkout clean: `{str(materializer['current_checkout'].get('clean_checkout', False)).lower()}`",
            "",
