...
Remaining gate: Real robot or accepted high-fidelity manifest/log/video/checkpoint evidence must pass strict audits. The strict fidelity acceptance provenance gate rejects acceptance files unless they come from guarded materialization, declare acceptance_ready=true, carry a SHA40 collection commit, carry an ISO-like lock date, include operator confirmation booleans, and keep strict evidence flags false until manifest-backed logs/videos pass. The pilot liveness layer and reset-timeout triage sidecar are non-evidence: they record timeout progress, the active reset-stage task/config/method/env, and whether a diagnostic sidecar rejected before JSONL write was stopped by the official video guard before any official row can be written.
