...
- Hardened `scripts/probe_maniskill_env_smoke.py` to isolate each candidate environment in its own subprocess; after installing the PartNet cabinet asset locally, the latest non-evidence smoke report shows all four primary ManiSkill/SAPIEN candidates construct/reset while support assets still need operator attention.
