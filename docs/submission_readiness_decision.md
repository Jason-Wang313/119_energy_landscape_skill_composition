...
- The ManiSkill pilot runtime liveness audit bounds the tracked reference runner in quarantined output directories and currently records a timeout before a complete schema-valid pilot row/video is produced while `pilot_runtime_ready=false`, explicit render-backend/shader controls are recorded, and render-backed RGB video remains unavailable; this is a pre-collection blocker, not validation evidence.
