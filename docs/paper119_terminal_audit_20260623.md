...
Updated artifact note: the current reframed PDF was rebuilt on 2026-07-01 as a 13-page paper with SHA256 `21D468E4AC3DC8E2705F1CE43536557FD8118049058EFE8F9A95F811C01B4582`.

The v5 expansion was completed as a CPU-only, RAM-light rebuild. The current artifact is a 13-page PDF in Downloads only, with blue citation/reference links, targeted skill-seam world/action-interface framing, diagnostic and decision-quality audits, and vector-first tables/figures generated from v5 CSV outputs.
...
Current reference-adapter note: `results/external_reference_adapter_audit.md` now verifies executable implementation-only adapters for all 12 methods, including 11 non-oracle baselines, while still treating them as non-evidence until real external logs, configs, videos, checkpoints, and manifest-declared implementations exist.

Current local-dry-run note: `results/external_local_dry_run_metrics.md` now verifies that the frozen local suite can be exported into 20,640 external-schema JSONL records and recomputed with the external rollout metric validator while still explicitly remaining non-evidence.
...
- Manifest-declared independent baseline evidence from real external runs.
