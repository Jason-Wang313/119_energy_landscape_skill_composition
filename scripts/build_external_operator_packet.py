...
def write_md(payload: dict[str, Any]) -> None:
    ...
    lines.extend(
        [
            ...
            f"- Plan-generation checkout commit: `{materializer['current_checkout'].get('code_commit', '')}`",
            f"- Plan-generation skill-library hash: `{materializer['current_checkout'].get('skill_library_hash', '')}`",
            f"- Plan-generation checkout clean: `{str(materializer['current_checkout'].get('clean_checkout', False)).lower()}`",
            "",
