...
- The ManiSkill pilot runtime liveness audit bounds the tracked reference runner in quarantined output directories and currently records `pilot_runtime_ready=false` after the runner reaches `record_video_start` but exits before a complete schema-valid pilot row/video is promoted; explicit render-backend/shader controls are recorded, render-backed RGB video remains unavailable, and this remains a pre-collection blocker rather than validation evidence.
