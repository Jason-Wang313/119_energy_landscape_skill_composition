...
- Keep the independent validation protocol, `results/external_operator_packet.md`, the ManiSkill fidelity metadata probe, the ManiSkill render-video preflight with its renderer-failure classifier, the reference-adapter provenance catalog, the strict reference-adapter rejection gate, and the manifest assembly checklist ready as follow-up artifacts, not as first-email clutter.
