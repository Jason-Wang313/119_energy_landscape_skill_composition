...
- The ManiSkill pilot runtime liveness audit bounds the tracked reference runner in quarantined output directories and currently records `pilot_runtime_ready=false`, zero schema-valid pilot rows, zero videos, and a renderer failure summary before official collection; this is a pre-collection blocker, not validation evidence.
