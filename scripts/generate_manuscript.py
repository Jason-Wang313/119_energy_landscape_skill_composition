...
def make_manuscript(summary):
    ...
    a(
        ...
        "We frame that handoff as a local world/action-modeling problem: a compact predictive interface between a skill library and a planner for handoff consequences. Given terminal-state evidence, next-skill basin/descent estimates, and a candidate transition, the interface estimates whether an action/skill composition will fail, diagnoses the likely reason, chooses accept, repair, probe, abstain, or alternate transition, and turns the observed result into planner-edge evidence so later plans can prefer reliable seams, repair uncertain ones, or avoid repeated failures. "
    ...
    a(
        "A reusable robot skill library still leaves a planning problem: the robot must know what the end of one skill makes possible for the next. We focus on that seam, the narrow place where an action model can be smaller than a full simulator but richer than a graph edge: it predicts what a completed skill makes true, risky, repairable, or impossible for the next skill. The model's job is not to operate either primitive or replace the planner; it is to make the transition legible enough for planning to choose the next action. At this scale, the useful prediction is not a full future video, but whether a specific transition creates a feasible start state, a repairable state, an uncertain state worth probing, or a state the planner should avoid."
    )
    a(
    ...
    a(
        "Concretely, the contribution is a seam-level predictive interface: estimate terminal/basin/barrier/descent/risk quantities, diagnose the likely failure mode, choose accept, repair, probe, abstain, or an alternate transition, and write the outcome back to planner-edge memory so future planning queries do not treat every graph edge as equally safe. The energy composer is the paper's implementation of that interface, not the identity of the contribution."
    )
