...
- PDF SHA256: `23120ABBBDA43A3F3D109F6F9895865F107D5895A336276193244C070B3D5B8D`.
- PDF size: `471761` bytes.
- PDF pages: `30`.
...
- ManiSkill reference backend readiness audit: `results/maniskill_backend_readiness_audit.md` exists, passes, and contract-qualifies an adapter-backed ManiSkill/SAPIEN backend candidate plus MP4 writer path while proving state-shaped arrays cannot masquerade as render videos; official collection and strict external evidence remain false.
...
- ManiSkill render-video preflight: `results/maniskill_render_video_preflight_audit.md` exists, passes as non-evidence tooling, probes render-backed RGB MP4 export for the tracked public-simulator route, and currently records render-video readiness as false on this local machine.
- ManiSkill pilot runtime liveness audit: `scripts/audit_maniskill_pilot_runtime_liveness.py` and `results/maniskill_pilot_runtime_liveness_audit.md` exist, pass as a bounded non-evidence liveness report, and currently record `pilot_runtime_ready=false` with one quarantined schema-valid pilot row/video produced through a diagnostic non-evidence fallback while render-backed RGB video remains unavailable before official collection.
