...
- Keep the independent validation protocol, `results/external_operator_packet.md`, the ManiSkill fidelity metadata probe, the fidelity acceptance materializer, the strict fidelity acceptance provenance gate, the ManiSkill render-video preflight with its renderer-failure classifier and timeout diagnosis retest, the ManiSkill render machine qualification packet, the ManiSkill pilot liveness guard that records whether a diagnostic sidecar rejected before JSONL write was stopped before promotion, the strict MP4 video evidence gate, the official video write guard, the official JSONL write guard, atomic official evidence promotion, the External ablation collection packet, the External evidence intake ledger, the External precollection manifest draft, the adapter acceptance fixtures, the reference-adapter provenance catalog, the strict reference-adapter rejection gate, the strict independent method provenance gate, the strict checkpoint/config artifact gate, the strict manifest promotion gate, and the manifest assembly checklist ready as follow-up artifacts, not as first-email clutter.
...
- Use the independent operator packet, strict fidelity acceptance provenance gate, strict MP4 video evidence gate, official video write guard, official JSONL write guard, atomic official evidence promotion, the pilot liveness guard that records whether a diagnostic sidecar rejected before JSONL write was stopped before promotion, External ablation collection packet, External evidence intake ledger, adapter acceptance fixtures, reference-adapter provenance catalog, strict reference-adapter rejection gate, strict independent method provenance gate, strict checkpoint/config artifact gate, strict manifest promotion gate, and manifest assembly checklist as Jason-owned validation scaffolding; do not frame Haonan as responsible for supplying the missing proof.
