...
- The ManiSkill pilot runtime liveness audit bounds the tracked reference runner in quarantined output directories and currently records `pilot_runtime_ready=false`, timeout progress, and whether a diagnostic sidecar rejected before JSONL write was stopped by the official video guard before a complete schema-valid pilot row/video is promoted; explicit render-backend/shader controls are recorded, render-backed RGB video remains unavailable, and this remains a pre-collection blocker rather than validation evidence.
- The external method implementation packet turns the missing non-oracle baseline layer into concrete per-method work orders, adapter acceptance fixtures, a reference-adapter provenance catalog, a strict reference-adapter rejection gate, a strict independent method provenance gate, and a strict checkpoint/config artifact gate while strict adapter evidence remains missing.
