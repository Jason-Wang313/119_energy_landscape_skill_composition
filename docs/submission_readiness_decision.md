...
- The ManiSkill pilot runtime liveness audit bounds the tracked reference runner in quarantined output directories and currently records `pilot_runtime_ready=false`, zero schema-valid pilot rows, and zero videos before timeout on the local CPU/render fallback path; this is a pre-collection blocker, not validation evidence.
