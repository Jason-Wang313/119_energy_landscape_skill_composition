...
Latest evidence-integrity hardening: the strict rollout uniqueness gate rejects duplicate episode identities or reused rollout videos, while the strict task-config hash gate and strict policy/config hash gate reject raw JSONL rollout rows whose task fields disagree with the manifest-declared config hash or whose `policy_or_config_hash` disagrees with the manifest-declared method `checkpoint_or_config_hash`.
