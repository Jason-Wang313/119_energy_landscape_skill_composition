...
- Use the independent operator packet, strict MP4 video evidence gate, official video write guard, official JSONL write guard, atomic official evidence promotion, External ablation collection packet, External evidence intake ledger, adapter acceptance fixtures, reference-adapter provenance catalog, strict reference-adapter rejection gate, strict independent method provenance gate, strict checkpoint/config artifact gate, strict manifest promotion gate, and manifest assembly checklist as Jason-owned validation scaffolding; do not frame Haonan as responsible for supplying the missing proof.
