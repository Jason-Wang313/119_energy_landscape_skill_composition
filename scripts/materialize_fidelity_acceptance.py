...
def write_md(payload: dict[str, Any]) -> None:
    lines = [
        ...
        "",
        f"- Plan-generation code commit: `{payload['current_checkout']['code_commit']}`",
        f"- Plan-generation skill-library hash: `{payload['current_checkout']['skill_library_hash']}`",
        f"- Plan-generation checkout clean: `{str(payload['current_checkout']['clean_checkout']).lower()}`",
        f"- Plan-generation dirty status lines: `{len(payload['current_checkout']['dirty_status_lines'])}`",
        "",
