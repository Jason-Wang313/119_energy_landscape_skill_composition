...
def make_manuscript(summary):
    ...
    a(
        "A reusable robot skill library still leaves a planning problem: the robot must know what the end of one skill makes possible for the next. This paper focuses on that seam, the narrow place where an action model can be smaller than a full simulator but richer than a graph edge. The model predicts what a completed skill makes true, risky, repairable, or impossible for the next skill. Its job is not to operate either primitive or replace the planner; it is to make the transition legible enough for planning to choose the next action. At this scale, the useful prediction is not a full future video, but whether a specific transition creates a feasible start state, a repairable state, an uncertain state worth probing, or a state the planner should avoid."
    )
    a(
