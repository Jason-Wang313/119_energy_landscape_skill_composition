...
- The external fidelity acceptance draft pre-fills the tracked ManiSkill route with platform, backend, config-hash, task-binding, and env-smoke anchors, but remains draft-only/non-evidence with all acceptance gates unaccepted.
...
- The generated external operator packet is a no-go handoff for independent validation: after materializing the high-fidelity route configs, it lists four generic pre-collection blockers, and it now exposes the tracked ManiSkill reference route where the backend/config/run-id/alias preflight has one remaining blocker, fidelity acceptance, without claiming evidence.
