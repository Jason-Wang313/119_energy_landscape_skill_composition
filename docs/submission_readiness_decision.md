...
- The ManiSkill env smoke probe records which bound simulator candidates construct/reset locally and which still need assets before backend qualification, while accepted fidelity and strict evidence remain false.
...
- The ManiSkill reference backend readiness audit contract-qualifies an adapter-backed ManiSkill/SAPIEN backend candidate, but official collection and strict external evidence remain false until an accepted backend writes real logs/videos under the strict gates.
