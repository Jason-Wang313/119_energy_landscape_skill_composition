        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Energy Landscape Skill Composition: Branch-Structured Evidence for Embodied World-Action Models}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study energy-based robot skill learning. The motivating bet is: Compose skills by compatible energy landscapes rather than sequencing policy modules.. We introduce a mechanism-level representation that keeps physically admissible but unrealized action outcomes as explicit branches. A synthetic contact-style evaluation shows that the proposed branch mechanism improves hidden-mode success and tail-risk relative to observed-only, augmentation-as-data, and scalar-uncertainty baselines.
        \end{abstract}

        \section{Introduction}
        Robotics papers often collapse unobserved physical alternatives into extra data, variance, or a single predicted next state. For energy-based robot skill learning, this can erase the difference between modes that are visually similar but action-critical. This paper asks whether a world-action model should expose those alternatives to the planner before execution.
...
\midrule
observed\_only\_wam & nominal & 0.455 & 0.545 \\
observed\_only\_wam & hidden\_mode\_shift & 0.152 & 0.848 \\
augmentation\_as\_data\_wam & nominal & 0.598 & 0.402 \\
augmentation\_as\_data\_wam & hidden\_mode\_shift & 0.280 & 0.720 \\
scalar\_uncertainty\_wam & nominal & 0.665 & 0.335 \\
scalar\_uncertainty\_wam & hidden\_mode\_shift & 0.323 & 0.677 \\
proposed\_branch\_mechanism & nominal & 0.735 & 0.265 \\
proposed\_branch\_mechanism & hidden\_mode\_shift & 0.530 & 0.470 \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        \section{Conclusion}
        Energy Landscape Skill Composition argues that embodied world-action models should preserve unrealized physical alternatives when those alternatives change downstream action value.
