...
def make_manuscript(summary):
    ...
    a(
        "Viewed this way, skill composition becomes a deliberately local form of adaptive physical world/action modeling for embodied agents: not a whole robot simulator, but a prediction-action-update loop at the handoff. This makes the seam a local world/action-modeling problem rather than a full-system modeling claim. That restraint is intentional. Rather than renaming every controller check as a world model, the paper studies the small interface where a planner asks, before committing to the next skill, what this transition is likely to do, why it might fail, which response is warranted, and what should be remembered afterward."
    )
    a(
